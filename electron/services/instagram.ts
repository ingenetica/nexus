...
const CALLBACK_PORT = 19848
const CALLBACK_PATH = '/callback'

export class InstagramClient extends SocialPlatform {
  ...
  isConnected(): boolean {
    const db = getDb()
    const account = db.prepare('SELECT id FROM social_accounts WHERE platform = ? LIMIT 1').get('instagram')
    return !!account
  }

  async connect(): Promise<void> {
    if (!this.appId || !this.appSecret) {
      throw new Error('Facebook/Instagram credentials not configured. Go to LLM Config to set your App ID and App Secret.')
    }
    ...
    authWindow.webContents.on('will-navigate', (event, url) => {
      ...
      const isFacebook = parsed.hostname.endsWith('facebook.com') || parsed.hostname.endsWith('instagram.com')
    ...
    const longLivedUrl = `https://graph.facebook.com/v18.0/oauth/access_token?grant_type=fb_exchange_token&client_id=${this.appId}&client_secret=${this.appSecret}&fb_exchange_token=${tokenData.access_token}`
    const longLivedResponse = await fetch(longLivedUrl)
    if (!longLivedResponse.ok) throw new Error(`Long-lived token exchange failed: ${longLivedResponse.statusText}`)
    ...
    // Get pages to find linked Instagram account
    ...
    // Store IG user ID in profile_url for later use
    db.prepare(`
      ...
      VALUES (?, 'instagram', ?, ?, ?, '', datetime('now', '+60 days'), datetime('now'))
    `).run(
      uuid(),
      `${pageName} (Instagram)`,
      igUserId,
    ...
    logger.info('ipc', 'Instagram account connected', { igUserId, pageName })
  }

  async disconnect(): Promise<void> {
    const db = getDb()
    db.prepare('DELETE FROM social_accounts WHERE platform = ?').run('instagram')
  }

  async publish(content: string, options?: PublishOptions): Promise<PublishResult> {
    const accountData = this.getAccountData()
    if (!accountData) return { success: false, error: 'Not connected to Instagram' }

    const { token, igUserId } = accountData

    try {
      // Instagram requires an image for feed posts
      if (!options?.imageUrl) {
        return { success: false, error: 'Instagram requires an image URL to publish. This article has no image.' }
      }
  ...
  async getComments(postExternalId: string): Promise<SocialComment[]> {
    ...
    try {
      ...
      const data = await response.json() as { data?: Array<{ id: string; from?: { username?: string }; text?: string; timestamp?: string }> }
  ...
  private getAccountData(): { token: string; igUserId: string } | null {
    const db = getDb()
    const account = db.prepare('SELECT access_token_encrypted, profile_url FROM social_accounts WHERE platform = ? LIMIT 1').get('instagram') as {
    ...
    if (!safeStorage.isEncryptionAvailable()) {
      logger.error('ipc', 'Cannot decrypt Instagram token: system encryption unavailable')
      return null
    }

    try {
      const token = safeStorage.decryptString(Buffer.from(account.access_token_encrypted, 'base64'))
      return { token, igUserId: account.profile_url }
    } catch {
      logger.error('ipc', 'Failed to decrypt Instagram access token')
      return null
    }
  }
  ...
  private waitForCallback(expectedState: string): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      server.listen(CALLBACK_PORT, '127.0.0.1', () => {
        logger.info('ipc', `Instagram OAuth callback server listening on 127.0.0.1:${CALLBACK_PORT}`)
      })
...
let instance: InstagramClient | null = null
...
export function resetInstagramClient(): void {
  instance = null
}
